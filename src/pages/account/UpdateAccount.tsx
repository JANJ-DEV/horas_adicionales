    ...
    <section className="flex flex-col gap-4 mt-8">
      ...
      <formAction.Form method="put" className="flex flex-col gap-4">
        <label htmlFor="displayName" className="flex flex-col gap-2">
          ...
          <input type="text" id="displayName" name="displayName" placeholder="Nombre de usuario" defaultValue={currentUser.displayName as string} />
        </label>
        <label htmlFor="uploadPhoto" className="flex gap-4 items-center">
            <img id="preview"
            src={formAction.data?.photoURL as string ? formAction.data?.photoURL : currentUser.photoURL as string}
         ...
         <small className="font-bold text-orange-300">Seleciona una foto de perfil</small>
          <input type="file" id="uploadPhoto" accept="image/*" name="uploadPhoto" title="Seleccionar foto de perfil" className="font-bold text-orange-300 hidden"/>
          </label>
        <button type="submit" className="py-2 px-4 border rounded-sm">Actualizar</button>
